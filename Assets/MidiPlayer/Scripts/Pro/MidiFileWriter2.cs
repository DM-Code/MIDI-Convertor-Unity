...
namespace MidiPlayerTK
{
    ...
    /// Adding MIDI event is done with method like MPTK_Addxxxxxxx. There is two kind of method: adding with a tick position (MPTK_AddNote) or with a time position (MPTK_AddNoteMilli).
    ...
    public class MidiFileWriter2
    {
        ...
        /// <summary>@brief
        /// @deprecated please investigate #MPTK_SignMap and #MPTK_CalculateTiming()
        /// </summary>
        public int MPTK_NumberBeatsMeasure { get { Debug.LogWarning("Deprecated, rather use MPTK_SignMap with MPTK_CalculateTiming"); return 0; } set { Debug.LogWarning("Deprecated, rather use MPTK_SignMap with MPTK_CalculateTiming"); } }

        /* useless 
        /// <summary>@brief
        /// From TimeSignature event: Describes of what note value a beat is (ie, how many quarter notes there are in a beat).
        /// @note
        ...
        */
        ...
        /// </summary>
        public double Bpm { get { Debug.LogWarning("Bpm is deprecated, rather use MPTK_Tempo"); return _bpm; } }
        ...
        /// <summary>@brief
        /// @deprecated  moved to class MptkEvent see MptkEvent.MPTK_BPM2MPQN
        /// </summary>
        public static int MPTK_GetMicrosecondsPerQuaterNote(int bpm)
        {
            Debug.LogWarning("MidiFileWriter2.MPTK_GetMicrosecondsPerQuaterNote is deprecated and will be removed. Please consider MptkEvent.MPTK_BPM2MPQN");
            return 60 * 1000 * 1000 / bpm;
        }

        /// <summary>@brief
        /// @deprecated moved to class MptkEvent replaced with MptkEvent.MPTK_BPM2MPQN
        /// </summary>
        public static int MPTK_BPM2MPQN(double bpm)
        {
            Debug.LogWarning("MidiFileWriter2.MPTK_BPM2MPQN is deprecated and will be removed. Please consider MptkEvent.MPTK_BPM2MPQN");
            return bpm > 0 ? (int)(60000000d / bpm) : 500000;
        }

        /// <summary>@brief
        /// @deprecated moved to class MptkEvent replaced with MptkEvent.MPTK_MPQN2BPM
        /// </summary>
        public static double MPTK_MPQN2BPM(int microsecondsPerQuaterNote)
        {
            Debug.LogWarning("MidiFileWriter2.MPTK_MPQN2BPM is deprecated and will be removed. Please consider MptkEvent.MPTK_MPQN2BPM");
        ...
        public float MPTK_ConvertTickToMilli(long tick, int indexTempo = -1)
        {
            if (indexTempo < 0)
            {
                MPTKTempo.MPTK_CalculateMap(MPTK_DeltaTicksPerQuarterNote, MPTK_MidiEvents, MPTK_TempoMap);
                indexTempo = MPTKTempo.MPTK_FindSegment(MPTK_TempoMap, tick, fromIndex: 0);
            }
            return (float)MPTK_TempoMap[indexTempo].MPTK_CalculateTime(tick);
        }
        ...
        public long MPTK_ConvertMilliToTick(float time, int indexTempo = -1)
        {
            if (indexTempo < 0)
            {
                MPTKTempo.MPTK_CalculateMap(MPTK_DeltaTicksPerQuarterNote, MPTK_MidiEvents, MPTK_TempoMap);
                indexTempo = MPTKTempo.MPTK_FindSegment(MPTK_TempoMap, time, fromIndex: 0);
            }
            return MPTK_TempoMap[indexTempo].MPTK_CalculatelTick(time);
        }

        /// <summary>@brief
        /// Convert the tick duration to a time duration in millisecond regarding the current tempo.\n
        /// @note Previous call to #MPTK_AddBPMChange and #MPTK_AddTempoChange have direct impact on this calculation.
        /// </summary>
        /// <param name="tick">duration in ticks</param>
        /// <returns>duration in milliseconds</returns>
        public long MPTK_DurationTickToMilli(long tick)
        {
        ...
        /// <param name="tick">duration in ticks</param>
        /// <returns>duration in milliseconds</returns>
        public long MPTK_DurationMilliToTick(float time)
        {
            return MPTK_PulseLenght > 0d ? (long)(time / MPTK_PulseLenght + 0.5f) : 0L;
        }
        ...
        /// @li real time, measure and beat for each events are recalculated for the whole MIDI events at the end off the import with #MPTK_CalculateTiming()
        ...
        public bool MPTK_ImportFromEventsList(List<MPTKEvent> midiEventsToInsert, int deltaTicksPerQuarterNote, long position = -1, string name = null, bool logPerf = false, bool logDebug = false)
        {
            bool ok = false;
            try
            {
                ...
                if (MPTK_MidiEvents.Count == 0)
                {
                    ...
                    MPTK_DeltaTicksPerQuarterNote = deltaTicksPerQuarterNote;
                    if (logDebug) Debug.Log($"Set MPTK_DeltaTicksPerQuarterNote from paremeter: {MPTK_DeltaTicksPerQuarterNote}");
                }
                ...
                if (position == 0)
                {
                    ...
                    if (MPTK_MidiEvents.Count != 0)
                    {
                        ...
                        if (logDebug) Debug.Log($"Shift {MPTK_MidiEvents.Count} source events, shift tick={shiftTick}");
                        ...
                        MPTK_MidiEvents.ForEach(midiEvent =>
                            {
                                ...
                                // No change on Length (tick duration) as the original DTPQN is not modified
                ...
                else if (position < 0 || position >= MPTK_MidiEvents.Last().Tick)
                {
                    ...
                    //shiftTime = insert.RealTime;
                    if (logDebug) Debug.Log($"Shift {midiEventsToInsert.Count} imported events, shift tick={shiftTick}");
                    if (logDebug) MPTK_AddText(0, insert.Tick, MPTKMeta.TextEvent, "Insert MIDI");
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (logDebug) Debug.Log($"Shift {midiEventsToInsert.Count} source events from postiion {indexToInsert}, shift tick={shiftTick} ");
                ...
                // -----------------------------------------------------
                MPTK_CalculateTiming(logPerf: logPerf);
        ...
        */
        ...
        /// <summary>@brief
        /// @deprecated with V2.9.0 rather use #MPTK_ImportFromEventsList.
        /// </summary>
        ...
        public MPTKEvent MPTK_AddRawEvent(MPTKEvent mptkEvent)
        {
            try
            {
        ...
        /// <returns>Return the MIDI event created or null if error</returns>
        public MPTKEvent MPTK_AddNoteMilli(int track, float timeToPlay, int channel, int note, int velocity, float duration)
        {
            try
            {
                long tick = MPTK_ConvertMilliToTick(timeToPlay);
                int length = duration < 0 ? -1 : (int)MPTK_DurationMilliToTick(duration);
                return MPTK_AddNote(track, tick, channel, note, velocity, length);
            }
            catch (System.Exception ex)
            {
                MidiPlayerGlobal.ErrorDetail(ex);
        ...
        /// <returns>Return the MIDI event created or null if error</returns>
        public MPTKEvent MPTK_AddNote(int track, long tick, int channel, int note, int velocity, int length)
        {
            ...
            try
            {
                ...
                if (length < 0)
                    ...
                    mptkEvent = MPTK_AddRawEvent(new MPTKEvent() { Track = track, Tick = tick, Channel = channel, Command = MPTKCommand.NoteOn, Value = note, Velocity = velocity, Duration = -1, Length = -1 });
                else
                {
                    long duration_ms = MPTK_DurationTickToMilli(length);
                    mptkEvent = MPTK_AddRawEvent(new MPTKEvent() { Track = track, Tick = tick, Channel = channel, Command = MPTKCommand.NoteOn, Value = note, Velocity = velocity, Duration = duration_ms, Length = length });
        ...
        /// <returns>Return the MIDI event created or null if error</returns>
        public MPTKEvent MPTK_AddSilenceMilli(int track, float timeToPlay, int channel, float duration)
        {
            try
            {
                long tick = MPTK_ConvertMilliToTick(timeToPlay);
                int durationSilence = duration <= 0 ? -1 : (int)MPTK_DurationMilliToTick(duration);
                return MPTK_AddSilence(track, tick, channel, durationSilence);
            }
            catch (System.Exception ex)
            {
                MidiPlayerGlobal.ErrorDetail(ex);
        ...
        /// <returns>Return the MIDI event created or null if error</returns>
        public MPTKEvent MPTK_AddSilence(int track, long tick, int channel, int length)
        {
            MPTKEvent mptkEvent = null;
            try
            {
        ...
        /// <param name="note">Note must be in the range 0-127</param>
        /// <returns>Return the MIDI event created or null if error</returns>
        public MPTKEvent MPTK_AddOffMilli(int track, float timeToPlay, int channel, int note)
        {
            try
            {
                long tick = MPTK_ConvertMilliToTick(timeToPlay);
                return MPTK_AddOff(track, tick, channel, note);
            }
            catch (System.Exception ex)
            {
                MidiPlayerGlobal.ErrorDetail(ex);
        ...
        /// <param name="note">Note must be in the range 0-127</param>
        /// <returns>Return the MIDI event created or null if error</returns>
        public MPTKEvent MPTK_AddOff(int track, long tick, int channel, int note)
        {
            MPTKEvent mptkEvent = null;
            try
            {
                ...
                for (int index = MPTK_MidiEvents.Count - 1; index >= 0; index--)
                {
                    ...
                    if (mptkEvent.Channel == channel && mptkEvent.Command == MPTKCommand.NoteOn && mptkEvent.Value == note)
                    {
                        ...
                        if (length > 0)
                        {
                            ...
                            mptkEvent.Duration = MPTK_DurationTickToMilli(length);
                            break;
                        }
                    }
                }
        ...
        /// <returns>Return the MIDI event created or null if error</returns>
        public MPTKEvent MPTK_AddChangePresetMilli(int track, float timeToPlay, int channel, int preset)
        {
            try
            {
                long tick = MPTK_ConvertMilliToTick(timeToPlay);
                return MPTK_AddChangePreset(track, tick, channel, preset);
            }
            catch (System.Exception ex)
            {
                MidiPlayerGlobal.ErrorDetail(ex);
        ...
        /// <param name="preset">Preset (program/patch) must be in the range 0-127</param>
        /// <returns>Return the MIDI event created or null if error</returns>
        public MPTKEvent MPTK_AddChangePreset(int track, long tick, int channel, int preset)
        {
            MPTKEvent mptkEvent = null;
            try
            {
        ...
        /// <returns>Return the MIDI event created or null if error</returns>
        public MPTKEvent MPTK_AddChannelAfterTouchMilli(int track, float timeToPlay, int channel, int afterTouchPressure)
        {
            try
            {
                long tick = MPTK_ConvertMilliToTick(timeToPlay);
                return MPTK_AddChannelAfterTouch(track, tick, channel, afterTouchPressure);
            }
            catch (System.Exception ex)
            {
                MidiPlayerGlobal.ErrorDetail(ex);
        ...
        /// <param name="afterTouchPressure">After-touch pressure from 0 to 127</param>
        /// <returns>Return the MIDI event created or null if error</returns>
        public MPTKEvent MPTK_AddChannelAfterTouch(int track, long tick, int channel, int afterTouchPressure)
        {
            MPTKEvent mptkEvent = null;
            try
            {
                ...
                return MPTK_AddRawEvent(mptkEvent);
            }
            catch (System.Exception ex)
            {
                MidiPlayerGlobal.ErrorDetail(ex);
        ...
        /// <returns>Return the MIDI event created or null if error</returns>
        public MPTKEvent MPTK_AddControlChangeMilli(int track, float timeToPlay, int channel, MPTKController controller, int controllerValue)
        {
            try
            {
                long tick = MPTK_ConvertMilliToTick(timeToPlay);
                return MPTK_AddControlChange(track, tick, channel, controller, controllerValue);
            }
            catch (System.Exception ex)
            {
                MidiPlayerGlobal.ErrorDetail(ex);
        ...
        /// <param name="controllerValue">Controller value</param>
        /// <returns>Return the MIDI event created or null if error</returns>
        public MPTKEvent MPTK_AddControlChange(int track, long tick, int channel, MPTKController controller, int controllerValue)
        {
            try
            {
        ...
        /// <returns>Return the MIDI event created or null if error</returns>
        public MPTKEvent MPTK_AddPitchWheelChangeMilli(int track, float timeToPlay, int channel, float pitchWheel)
        {
            try
            {
                long tick = MPTK_ConvertMilliToTick(timeToPlay);
                return MPTK_AddPitchWheelChange(track, tick, channel, pitchWheel);
            }
            catch (System.Exception ex)
            {
                MidiPlayerGlobal.ErrorDetail(ex);
        ...
        /// <returns>Return the MIDI event created or null if error</returns>
        public MPTKEvent MPTK_AddPitchWheelChange(int track, long tick, int channel, float pitchWheel)
        {
            try
            {
                int pitch = (int)Mathf.Lerp(0f, 16383f, pitchWheel); // V2.88.2 range normalized from 0 to 1
                                                                     //Debug.Log($"{pitchWheel} --> {pitch}");
                return MPTK_AddRawEvent(new MPTKEvent() { Track = track, Tick = tick, Channel = channel, Command = MPTKCommand.PitchWheelChange, Value = pitch });
            }
            catch (System.Exception ex)
            {
                MidiPlayerGlobal.ErrorDetail(ex);
        ...
        /// <returns>Return the MIDI event created or null if error</returns>
        public MPTKEvent MPTK_AddBPMChange(int track, long tick, int bpm)
        {
            if (bpm <= 0)
            {
            ...
            try
            {
                ...
                return MPTK_AddRawEvent(new MPTKEvent() { Track = track, Tick = tick, Command = MPTKCommand.MetaEvent, Meta = MPTKMeta.SetTempo, Value = MPTK_MicrosecondsPerQuaterNote/*, Duration = _bpm*/ });
            }
            catch (System.Exception ex)
            {
                MidiPlayerGlobal.ErrorDetail(ex);
        ...
        /// <returns>Return the MIDI event created or null if error</returns>
        public MPTKEvent MPTK_AddTempoChange(int track, long tick, int microsecondsPerQuarterNote)
        {
            ...
            try
            {
                _bpm = MPTKEvent.MPTK_MPQN2BPM(microsecondsPerQuarterNote);
                //Value contains new Microseconds Per Beat Note and Duration contains new tempo (quarter per minute).
                return MPTK_AddRawEvent(new MPTKEvent() { Track = track, Tick = tick, Command = MPTKCommand.MetaEvent, Meta = MPTKMeta.SetTempo, Value = microsecondsPerQuarterNote,/* Duration = _bpm*/ });
            }
            catch (System.Exception ex)
            {
                MidiPlayerGlobal.ErrorDetail(ex);
        ...
        /// <returns>Return the MIDI event created or null if error</returns>
        public MPTKEvent MPTK_AddTimeSignature(int track, long tick, int numerator = 4, int denominator = 2, int ticksInMetronomeClick = 24, int no32ndNotesInQuarterNote = 32)
        {
            try
            {
                ...
                return MPTK_AddRawEvent(new MPTKEvent()
                {
                    Track = track,
                    Tick = tick,
        ...
        /// <returns>Return the MIDI event created or null if error</returns>
        public MPTKEvent MPTK_AddTextMilli(int track, float timeToPlay, MPTKMeta typeMeta, string text)
        {
            try
            {
                long tick = MPTK_ConvertMilliToTick(timeToPlay);
                return MPTK_AddText(track, tick, typeMeta, text);
            }
        ...
        /// <param name="text">The text associated to this MIDI event</param>
        /// <returns>Return the MIDI event created or null if error</returns>
        public MPTKEvent MPTK_AddText(int track, long tick, MPTKMeta typeMeta, string text)
        {
            try
            {
                switch (typeMeta)
                {
                    ...
                    case MPTKMeta.TrackInstrumentName:
                        return MPTK_AddRawEvent(new MPTKEvent() { Track = track, Tick = tick, Command = MPTKCommand.MetaEvent, Meta = typeMeta, Info = text });
                    default:
        ...
        public void MPTK_StableSortEvents(bool logPerf = false)
        {
            if (MPTK_MidiEvents != null)
            {
                ...
                Debug.LogWarning("MidiFileWriter2 - MPTK_SortEvents - MPTK_MidiEvents is null");
        }
        ...
        /// <param name="logPerf"></param>
        /// <param name="logDebug"></param>
        public void MPTK_CalculateTiming(bool logPerf = false, bool logDebug = false)
        {
            if (MPTK_MidiEvents != null)
            {
                System.Diagnostics.Stopwatch watch = null;
                ...
                //    MPTK_TempoMap.Add(new MPTKTempo(index: MPTK_TempoMap.Count, fromTick: 0, microsecondsPerQuarterNote: MptkEvent.MPTK_BPM2MPQN(120),
                //        pulse: (double)MptkEvent.MPTK_BPM2MPQN(120) / (double)MPTK_DeltaTicksPerQuarterNote / 1000d));
                //}
                ...
                int indexTempo = 0;
                int indexSign = 0;
                if (logDebug) Debug.Log($"MPTK_CalculateTiming index:{indexTempo} {MPTK_TempoMap[indexTempo]} {MPTK_SignMap[indexSign]} (init)");
                foreach (MPTKEvent mptkEvent in MPTK_MidiEvents)
                {
                    mptkEvent.Index = indexEvent++;

                    indexTempo = MPTKTempo.MPTK_FindSegment(MPTK_TempoMap, mptkEvent.Tick, fromIndex: indexTempo);
                    MPTKTempo tempoMap = MPTK_TempoMap[indexTempo];
                    mptkEvent.RealTime = (float)tempoMap.MPTK_CalculateTime(mptkEvent.Tick);
                    if (mptkEvent.Command == MPTKCommand.NoteOn && mptkEvent.Duration > -1)
                        mptkEvent.Duration = (long)(mptkEvent.Length * tempoMap.Pulse);

                    indexSign = MPTKSignature.MPTK_FindSegment(MPTK_SignMap, mptkEvent.Tick, fromIndex: indexSign);
                    MPTKSignature signMap = MPTK_SignMap[indexSign];
                    mptkEvent.Measure = signMap.MPTK_TickToMeasure(mptkEvent.Tick);
                    mptkEvent.Beat = signMap.MPTK_CalculateBeat(mptkEvent.Tick, mptkEvent.Measure);
                    if (logDebug) Debug.Log($"MPTK_CalculateTiming index:{indexTempo} {MPTK_TempoMap[indexTempo]} {MPTK_SignMap[indexSign]}");
                }
                if (logPerf)
                {
                    Debug.Log($"MPTK_CalculateTiming {watch.ElapsedMilliseconds} ms {watch.ElapsedTicks} timer ticks");
                    watch.Stop();
                }
            }
            else
                Debug.LogWarning("MidiFileWriter2 - MPTK_CalculateTiming - MPTK_MidiEvents is null");
        }
