    ...
    public partial class MidiEditorWindow : EditorWindow
    {
        ...
        private void CreateMidi()
        {
            if (CheckMidiSaved())
            {
                ...
                MidiFileWriter.MPTK_AddTempoChange(0, 0, MPTKEvent.MPTK_BPM2MPQN(MidiFileWriter.MPTK_Tempo));
                MidiFileWriter.MPTK_AddTimeSignature(0, 0, numerator: 4, denominator: 2, ticksInMetronomeClick: 24);
                MPTKTempo.MPTK_CalculateMap(MidiFileWriter.MPTK_DeltaTicksPerQuarterNote, MidiFileWriter.MPTK_MidiEvents, MidiFileWriter.MPTK_TempoMap);
                MPTKSignature.MPTK_CalculateMap(MidiFileWriter.MPTK_DeltaTicksPerQuarterNote, MidiFileWriter.MPTK_MidiEvents, MidiFileWriter.MPTK_SignMap);
                MPTKSignature.MPTK_CalculateMeasureBoundaries(MidiFileWriter.MPTK_SignMap);
                //! [ExampleInitMidiFileWriter]
                ...
                AddSectionMeta();
                MidiEventEdit.Length = MidiFileWriter.MPTK_DeltaTicksPerQuarterNote;
                InitPosition();
        ...
        private void ResetCurrent()
        {
            ...
            MidiEventEdit.Length = MidiFileWriter != null ? MidiFileWriter.MPTK_DeltaTicksPerQuarterNote : 500;
        ...
        private void ImportToMidiFileWriter(MidiLoad midiLoader)
        {
            ...
            try
            {
                ...
                MidiFileWriter.MPTK_CalculateTiming();
                MidiEvents = MidiFileWriter.MPTK_MidiEvents;
                SelectedEvent = null;
            }
            catch (Exception ex)
        ...
        private void SetMuteChannel()
        {
            for (int channel = 0; channel < 16; channel++)
                Player.MPTK_ChannelEnableSet(channel, !Context.SectionMute[channel]);
        }
        ...
        private void CmdNewLoadSaveTools(float startX, float starty, float width, float height)
        {
            try
            {
                ...
                if (MidiFileWriter != null)
                {
                    string label = $"DTPQN:{MidiFileWriter.MPTK_DeltaTicksPerQuarterNote}  {MidiFileWriter.MPTK_SignMap[0].NumberBeatsMeasure}/{MidiFileWriter.MPTK_SignMap[0].NumberQuarterBeat}  Events:{MidiFileWriter.MPTK_MidiEvents.Count}   ";
                ...
                {
                    MidiFileWriter.MPTK_CalculateTiming(logPerf: true);
        ...
        private void CmdGoto()
        {
            ...
            {
                int index = MPTKSignature.MPTK_FindSegment(MidiFileWriter.MPTK_SignMap, current);
                int measure = MidiFileWriter.MPTK_SignMap[index].MPTK_TickToMeasure(current);
                if (measure > 0)
                {
                    long tick = MPTKSignature.MPTK_MeasureToTick(MidiFileWriter.MPTK_SignMap, measure - 1);
                    if (tick >= 0)
                        current = tick;
                }
            }
                ...
                if (MidiFileWriter.MPTK_TempoMap.Count > 0)
                {
                    try
                    {
                        if (LastMidiEvent != null)
                            lastPositionMilli = (int)(MidiFileWriter.MPTK_TempoMap.Last().MPTK_CalculateTime(LastMidiEvent.Tick) / Player.MPTK_Speed);
                        indexTempo = MPTKTempo.MPTK_FindSegment(MidiFileWriter.MPTK_TempoMap, current, fromIndex: 0);
                        currentPositionMilli = (int)(MidiFileWriter.MPTK_TempoMap[indexTempo].MPTK_CalculateTime(current) / Player.MPTK_Speed);
                    }
                    catch
                    {
        ...
        private void RefreshPadCmdMidi(MPTKEvent mptkEvent)
        {
            ...
            else if (mptkEvent.Command == MPTKCommand.MetaEvent)
            {
                ...
                if (mptkEvent.Meta == MPTKMeta.SetTempo)
                {
                    ...
                    MidiEventEdit.TempoBpm = (int)MPTKEvent.MPTK_MPQN2BPM(mptkEvent.Value);
                }
        ...
        private void RefreshMidi(bool logPerf = true)
        {
            ...
            MidiFileWriter.MPTK_StableSortEvents(logPerf: true);
            MidiFileWriter.MPTK_CalculateTiming(logPerf: true, logDebug: false);
            FindLastMidiEvents();
            sectionAll.InitSections();
            Context.Modified = true;
            if (logPerf)
            ...
            try // Begin area measure line
            {
                ...
                if (Context.QuarterWidth > 1f) // To avoid infinite loop
                {
                    ...
                    for (float xQuarter = 0; xQuarter <= SectionChannel.FullWidthSections; xQuarter += Context.QuarterWidth)
                    {
                        ...
                        tick = (long)((tick / (float)MidiFileWriter.MPTK_DeltaTicksPerQuarterNote) + 0.5f) * MidiFileWriter.MPTK_DeltaTicksPerQuarterNote;
                        indexSign = MPTKSignature.MPTK_FindSegment(MidiFileWriter.MPTK_SignMap, tick, fromIndex: indexSign);
        ...
        {
            int tick = quarter * MidiFileWriter.MPTK_DeltaTicksPerQuarterNote;
            int indexTempo = 0;
            double realTime = 0;
            ...
            {
                //! [ExampleFindTempoMap]
                indexTempo = MPTKTempo.MPTK_FindSegment(MidiFileWriter.MPTK_TempoMap, tick, fromIndex: 0);
                realTime = MidiFileWriter.MPTK_TempoMap[indexTempo].MPTK_CalculateTime(tick) / 1000f / Player.MPTK_Speed;
                //! [ExampleFindTempoMap]
            }
            catch
            {
                        ...
                        else if (layout.Type == Layout.EnumType.Meta)
                        {
                            ...
                            if (line == 0)
                                newEvent = new MPTKEvent()
                                {
                                    ...
                                    /// MPTKEvent#Value contains new Microseconds Per Beat Note\n
                                    Value = MPTKEvent.MPTK_BPM2MPQN(MidiEventEdit.TempoBpm),
                                };
                            else if (line == 1)
                                newEvent = new MPTKEvent()
                                {
        ...
        private void EventWheelScrollH(Event currentEvent)
        {
            ...
            CurrentTickPosition -= (long)((MidiFileWriter != null ? MidiFileWriter.MPTK_DeltaTicksPerQuarterNote * 4f : 1000f) * currentEvent.delta.y / 3f);
