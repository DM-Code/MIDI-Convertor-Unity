...

{
    ...
    public class TestMidiGenerator : MonoBehaviour
    {
        ...
        private void GUIExample(Func<MidiFileWriter2> _mfwGenerator, string _title, string _filename)
        {
            float height = 40;
            float width = 100;
        ...
        private MidiFileWriter2 CreateMidiStream_four_notes_milli()
        {
            ...
            mfw.MPTK_AddText(track0, 0, MPTKMeta.Copyright, "Simple MIDI Generated. 4 quarter ");
            ...
            mfw.MPTK_AddNoteMilli(track1, timeToPlay: 0f, channel0, 60, 50, duration: 500f);
            mfw.MPTK_AddNoteMilli(track1, timeToPlay: 500f, channel0, 61, 50, duration: 500f);
            mfw.MPTK_AddNoteMilli(track1, timeToPlay: 1000f, channel0, 62, 50, duration: 500f);
            mfw.MPTK_AddNoteMilli(track1, timeToPlay: 1500f, channel0, 63, 50, duration: 500f);
        ...
        private MidiFileWriter2 CreateMidiStream_four_notes_ticks()
        {
            ...
            mfw.MPTK_AddTimeSignature(0, 0, 4, 2);
        ...
        private MidiFileWriter2 CreateMidiStream_Chords()
        {
            ...
            mfw.MPTK_AddChangePreset(track1, absoluteTime, channel0, preset: 40);
            ...
            long duration = (long)mfw.MPTK_DurationTickToMilli(ticksPerQuarterNote);
            ...
            // Piano
            mfw.MPTK_AddChangePreset(track1, absoluteTime, channel0, preset: 0);
        ...
        private MidiFileWriter2 CreateMidiStream_sandbox()
        {
            ...
            mfw.MPTK_AddChangePreset(track1, 0, channel0, preset: 65); // alto sax
            mfw.MPTK_AddChangePreset(track1, 0, channel1, preset: 18); // rock organ
        ...
        /// </summary>
        /// <returns></returns>
        private MidiFileWriter2 CreateMidiStream_200_drum_hit_at_each_quarter()
        {
            ...
            for (int iQuarter = 0; iQuarter <= 200; iQuarter += 1)
            {
                ...
                mfw.MPTK_AddNote(track: 1, absoluteTime, channel: 9, note: 37, velocity: 70, length: ticksPerQuarterNote / 2);
                
                // Low floor Tom at each bar
                if (iQuarter % 4 == 0)
                    mfw.MPTK_AddNote(track: 1, absoluteTime, channel: 9, note: 41, velocity: 120, length: ticksPerQuarterNote / 2);
        ...
        /// </summary>
        /// <returns></returns>
        private MidiFileWriter2 CreateMidiStream_tempochange_milli()
        {
            MidiFileWriter2 mfw = new MidiFileWriter2(deltaTicksPerQuarterNote: 500);
            // Initial tempo change
            mfw.MPTK_AddBPMChange(1, 0, 60);
            mfw.MPTK_AddNoteMilli(1, timeToPlay: 0.0f, 0, 60, 60, duration: 1f);
            mfw.MPTK_AddNoteMilli(1, timeToPlay: 1.0f, 0, 61, 60, duration: 1f);
            mfw.MPTK_AddNoteMilli(1, timeToPlay: 2.0f, 0, 62, 60, duration: 1f);
            mfw.MPTK_AddNoteMilli(1, timeToPlay: 3.0f, 0, 63, 60, duration: 1f);

            // simulate tempo change to 120
            mfw.MPTK_AddNoteMilli(1, timeToPlay: 4.0f, 0, 60, 60, duration: 0.5f);
            mfw.MPTK_AddNoteMilli(1, timeToPlay: 4.5f, 0, 61, 60, duration: 0.5f);
            mfw.MPTK_AddNoteMilli(1, timeToPlay: 5.0f, 0, 62, 60, duration: 0.5f);
            mfw.MPTK_AddNoteMilli(1, timeToPlay: 5.5f, 0, 63, 60, duration: 0.5f);

            // simulate tempo change to 240
            mfw.MPTK_AddNoteMilli(1, timeToPlay: 6.0f, 0, 60, 60, duration: 0.25f);
            mfw.MPTK_AddNoteMilli(1, timeToPlay: 6.25f, 0, 61, 60, duration: 0.25f);
            mfw.MPTK_AddNoteMilli(1, timeToPlay: 6.50f, 0, 62, 60, duration: 0.25f);
            mfw.MPTK_AddNoteMilli(1, timeToPlay: 6.75f, 0, 63, 60, duration: 0.25f);
            return mfw;
        }
        /// <summary>@brief
        ...
        {
            MidiFileWriter2 mfw = new MidiFileWriter2(deltaTicksPerQuarterNote: 500);
            mfw.MPTK_AddText(0, tick: 0, MPTKMeta.Lyric, "some text");
            mfw.MPTK_AddNote(1, tick: 500, 0, 61, 100, 500);
            mfw.MPTK_AddNote(1, tick: 0, 0, 60, 100, 500);
            mfw.MPTK_AddChangePreset(1, tick: 0, 0, preset: 100);
            mfw.MPTK_AddChangePreset(1, tick: 10, 0, preset: 100);
            mfw.MPTK_AddText(0, tick: 0, MPTKMeta.Lyric, "other text");
        ...
        private MidiFileWriter2 CreateMidiStream_silence()
        {
            ...
            mfw.MPTK_AddChangePreset(track: 1, absoluteTime, channel: 0, preset: 21);
        ...
        private MidiFileWriter2 CreateFourOnTheFloor()
        {
            int track0 = 0;
            int track1 = 1;
            ...
            mfw.MPTK_AddNote(track1, absoluteTime, channel0, note: 40, velocity: 100, length: -1);
            absoluteTime += ticksPerQuarterNote;
            ...
            mfw.MPTK_AddNote(track1, absoluteTime, channel0, note: 36, velocity: 100, length: -1);
            absoluteTime += ticksPerQuarterNote;

            mfw.MPTK_AddNote(track1, absoluteTime, channel0, note: 36, velocity: 1, length: -1);
            return mfw;
        }



        private MidiFileWriter2 CreateMultipleTimeSignature()
        {
            // In this demo, we are using variable to contains tracks and channel values only for better understanding. 
            ...
            ////      Numerator(number of beats in a bar, 
            ////      Denominator(which is confusingly) in 'beat units' so 1 means 2, 2 means 4(crochet), 3 means 8(quaver), 4 means 16 and 5 means 32), 
            mfw.MPTK_AddTimeSignature(track: 0, tick: absoluteTime, numerator: 4, denominator: 2); // for a 4/4 signature
            mfw.MPTK_AddText(track: 0, absoluteTime, MPTKMeta.TextEvent, "TimeSignature 4/2");


            // Tempo is defined in beat per minute (not mandatory, by default MIDI are played with a tempo of 120).
            // beatsPerMinute set to 60 at start, it's a slow tempo, one quarter per second.
            // Tempo is global for the whole MIDI independantly of each track and channel.
            mfw.MPTK_AddBPMChange(track: 0, tick: absoluteTime, bpm: 120);

            // Build bar
            for (int i = 0; i < 4; i++)
            {
                mfw.MPTK_AddNote(track: 1, tick: absoluteTime, channel: 0, note: 60 + i, velocity: 100, length: ticksPerQuarterNote);
                absoluteTime += ticksPerQuarterNote; // Next note will be played one quarter after the previous (time signature is 4/4)
            }

            mfw.MPTK_AddTimeSignature(track: 0, tick: absoluteTime, numerator: 2, denominator: 2); // for a 2/4 signature
            mfw.MPTK_AddText(track: 0, absoluteTime, MPTKMeta.TextEvent, "TimeSignature 2/2");

            // Build bar
            for (int i = 0; i < 4; i++)
            {
                mfw.MPTK_AddNote(track: 1, tick: absoluteTime, channel: 0, note: 55 + i, velocity: 100, length: ticksPerQuarterNote);
                absoluteTime += ticksPerQuarterNote; // Next note will be played one quarter after the previous (time signature is 4/4)
            }

            mfw.MPTK_AddTimeSignature(track: 0, tick: absoluteTime, numerator: 1, denominator: 2); // for a 2/4 signature
            mfw.MPTK_AddText(track: 0, absoluteTime, MPTKMeta.TextEvent, "TimeSignature 1/2");

            // Build bar
            for (int i = 0; i < 4; i++)
            {
                mfw.MPTK_AddNote(track: 1, tick: absoluteTime, channel: 0, note: 50 + i, velocity: 100, length: ticksPerQuarterNote);
                absoluteTime += ticksPerQuarterNote; // Next note will be played one quarter after the previous (time signature is 4/4)
            }

            mfw.MPTK_AddTimeSignature(track: 0, tick: absoluteTime, numerator: 4, denominator: 2); // for a 2/4 signature
            mfw.MPTK_AddText(track: 0, absoluteTime, MPTKMeta.TextEvent, "TimeSignature 4/2");
        ...
        /// </summary>
        /// <returns></returns>
        private MidiFileWriter2 CreateMidiStream_time_signature_4_1()
        {
            int ticksPerQuarterNote = 500;
            ...
            mfw.MPTK_AddBPMChange(track: 0, absoluteTime, 120);
            mfw.MPTK_AddTimeSignature(track: 1, absoluteTime, numerator: 4, denominator: 1);
            // Generate 16 notes
            for (int note = 0; note < 16; note++)
            {
                // Duration = 0.25 second for a quarter at BPM 240
        ...
        /// </summary>
        /// <returns></returns>
        private MidiFileWriter2 CreateMidiStream_time_signature_4_3()
        {
            int ticksPerQuarterNote = 500;
            ...
            mfw.MPTK_AddBPMChange(track: 0, absoluteTime, 120);
            mfw.MPTK_AddTimeSignature(track: 1, absoluteTime, numerator: 4, denominator: 3);
            // Generate 16 notes
            for (int note = 0; note < 16; note++)
            {
                // Duration = 0.25 second for a quarter at BPM 240
        ...
        private void PlayDirectlyMidiSequence(string name, MidiFileWriter2 mfw)
        {
            ...
            mfw.MPTK_CalculateTiming(logPerf: true);
